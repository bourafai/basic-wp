...
class Helpers {
	...
	public static function locate_template( $tpl ) {
		...
		$path = apply_filters( 'BEA/Helpers/locate_template/templates', array( 'views/' . AB_POSTS_RENDER_VIEWS_FOLDER_NAME . '/' . $tpl . '.php' ), $tpl, __NAMESPACE__ );
		...
		// Locate on the files
		if ( is_file( AB_POSTS_RENDER_DIR . 'views/' . $tpl . '.php' ) ) {// Use builtin template
			return ( AB_POSTS_RENDER_DIR . 'views/' . $tpl . '.php' );
		}
